...
set(ByVal Value As Boolean)
    ...
    Public ReadOnly Property FullName(Optional ByVal TheName As Boolean = False) As String
        {
            get
            {
            if (arlNames.Count > 0)
            {
                private string sArticle2 = sArticle;
                If TheName Then sArticle2 = "the"
                if (sPrefix <> "")
                {
...
set(ByVal Value As String)
    ...
    'Friend ReadOnly Property DescriptionInGame() As String
    '    Get
    '        Dim sReturn As String = Description.ToString

    '        If Openable Then
    '            sReturn = pSpace(sReturn) ' If sReturn <> "" Then sReturn &= "  "
    '            If IsOpen Then
    '                sReturn &= ToProper(FullName(True)) & " is open."
    '            Else
    '                sReturn &= ToProper(FullName(True)) & " is closed."
    '            End If
    '        End If

    '        sReturn &= DisplayObjectChildren()
    '        sReturn &= DisplayCharacterChildren()

    '        If sReturn = "" AndAlso Key <> "" Then sReturn = "There is nothing interesting about " & Me.FullName & "."
    ...
    public string DisplayCharacterChildren()
    {
        ...
        if (ChildrenCharacters(WhereChildrenEnum.OnObject).Count > 0)
        {
            ...
            sReturn &= ChildrenCharacters(WhereChildrenEnum.OnObject).List("and") ' ToProper;
        ...
        if ((Openable && IsOpen) || Not Openable)
        {
            if (ChildrenCharacters(WhereChildrenEnum.InsideObject).Count > 0)
            {
                ...
                Else
                    sReturn = pSpace(sReturn)
                    sReturn &= ChildrenCharacters(WhereChildrenEnum.InsideObject).List("and") ' ToProper;
                }
    ...
    public string DisplayObjectChildren()
    {
        ...
        if (Children(WhereChildrenEnum.OnObject).Count > 0)
        {
            ...
            sReturn = pSpace(sReturn)
            sReturn &= ToProper(Children(WhereChildrenEnum.OnObject).List("and", false, false));
        ...
        if ((Openable && IsOpen) || Not Openable)
        {
            if (Children(WhereChildrenEnum.InsideObject).Count > 0)
            {
                ...
                sReturn &= Children(WhereChildrenEnum.InsideObject).List("and", false, false);
            }
        }
        ...
        If sReturn = "" Then sReturn = "null is on or inside " + FullName(true) + "."
        return sReturn;
    ...
    internal Description Description { get; set; }
        {
            ...
            {
                oDescription(0).Description = "There is nothing special about " + FullName(true) + ".";
            }
#endif
            return oDescription;
        }
    ...
    Public ReadOnly Property IsVisibleTo(ByVal sCharKey As String) As Boolean
        {
            ...
            {
            If sCharKey = "%Player%" Then sCharKey = Adventure.Player.Key
            private clsCharacterLocation locChar = Adventure.htblCharacters(sCharKey).Location;
            switch (locChar.ExistWhere)
            {
                ...
                case clsCharacterLocation.ExistsWhereEnum.InObject:
                    ...
                    if (Adventure.htblObjects(locChar.Key).IsOpen)
                    {
                    ...
                    Else
                        ...
                        return locChar.Key = Me.Key || Adventure.htblObjects(locChar.Key).Children(WhereChildrenEnum.InsideObject, true).ContainsKey(Me.Key);
                    }
                ...
                Else
                    switch (.DynamicExistWhere)
                    {
                        ...
                        case clsObjectLocation.DynamicExistsWhereEnum.InObject:
                            ...
                            if (.Key <> "")
                            {
                                private clsObject parent = Adventure.htblObjects(.Key);
                                if (parent.IsVisibleAtLocation(sLocationKey) Then ' parent.IsVisibleTo(sCharKey))
                                {
                                    if (Not parent.Openable || parent.IsOpen)
                                    {
                                        return true;
                                    }
                                }
                        ...
                        case clsObjectLocation.DynamicExistsWhereEnum.OnObject:
                            {
                            If .Key <> "" Then Return Adventure.htblObjects(.Key).IsVisibleAtLocation(sLocationKey) ' .IsVisibleTo(sCharKey)
                        case clsObjectLocation.DynamicExistsWhereEnum.WornByCharacter:
                            {
                            If .Key <> "" Then Return Adventure.htblCharacters(.Key).IsVisibleAtLocation(sLocationKey)
                            'If .Key <> "" Then Return sLocationKey = Adventure.htblCharacters(.Key).Location.Key AndAlso Adventure.htblCharacters(.Key).Location.ExistWhere = clsCharacterLocation.ExistsWhereEnum.AtLocation
    ...
    Friend ReadOnly Property Children(ByVal WhereChildren As WhereChildrenEnum, Optional ByVal bRecursive As Boolean = False) As ObjectHashTable
        {
            ...
            For Each ob As clsObject In Adventure.htblObjects.Values
                if ((WhereChildren <> WhereChildrenEnum.OnObject && ob.Location.DynamicExistWhere = clsObjectLocation.DynamicExistsWhereEnum.InObject) || (WhereChildren <> WhereChildrenEnum.InsideObject && ob.Location.DynamicExistWhere = clsObjectLocation.DynamicExistsWhereEnum.OnObject))
                {
                    if (ob.Location.Key = Me.Key)
                    {
                        ...
                        if (bRecursive)
                        {
                            For Each obChild As clsObject In ob.Children(WhereChildrenEnum.InsideOrOnObject, True).Values
...
public enum DynamicExistsWhereEnum
    ...
    public DynamicExistsWhereEnum DynamicExistWhere;
...
public enum StaticExistsWhereEnum
    ...
    public StaticExistsWhereEnum StaticExistWhere;
    ...
    public string Key { get; set; }
        {
            get
            {
            If sKey = "%Player%" && Adventure.Player != null Then sKey = Adventure.Player.Key
            return sKey;
        }
set(ByVal Value As String)
            sKey = Value
