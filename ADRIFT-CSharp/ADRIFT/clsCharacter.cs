...
set(ByVal Value As Boolean)
    ...
    'Private bFirstMention As Boolean = True
    'Public bTurnMention As Boolean = False
#if Runner
    Friend ReadOnly Property Name(Optional ByVal Pronoun As PronounEnum = PronounEnum.Subjective, Optional bMarkAsSeen As Boolean = True, Optional ByVal bAllowPronouns As Boolean = True, Optional ByVal bForceTheName As Boolean = False) As String
        {
            get
            {
            With UserSession;
                ...
                private bool bReplaceWithPronoun = false;
                private bool bTheName = bForceTheName;

                if (.bDisplaying)
                {
                    'If Adventure.lCharMentionedThisTurn(Gender).Contains(Key) Then bTheName = True
                    bTheName = bTheName OrElse Introduced
                    if (bMarkAsSeen)
                    {
                        Introduced = True
                    ...
                    if (.PronounKeys.Count > 0)
                    {
                        ...
                        {
                            With .PronounKeys(i);
                                If .Gender = Gender && .Key = Key Then ' && .Pronoun = Pronoun
                ...
                if (Not bAllowPronouns || Not bReplaceWithPronoun)
                {
                    ...
                    if (HasProperty("Known"))
                    {
                        if (GetProperty("Known").Selected)
                        {
                            sName = ProperName
                        Else
                            sName = Descriptor(bTheName) 'Not bFirstMention)
                            'If bDisplaying AndAlso Not bForceName Then bFirstMention = False
                            'Return sName
                        }
                    Else
                        if (Descriptor <> "")
...
#else
    Friend ReadOnly Property Name(optional byval a as boolean=false, optional byval b as boolean=false, optional byval c as boolean=false, optional byval d as boolean = false) As String
        {
            get
            {
            if (ProperName <> "")
...
set(ByVal Value As String)
    ...
    Public ReadOnly Property Descriptor(Optional ByVal TheName As Boolean = False) As String
        {
            get
            {
            if (arlDescriptors.Count > 0)
            {
                ...
                If TheName && sArticle <> "" Then sArticle2 = "the"
    ...
    Public ReadOnly Property CanSeeCharacter(ByVal sCharKey As String) As Boolean
        {
            get
            {
            if (Adventure.htblCharacters.ContainsKey(sCharKey))
            {
    ...
    Public ReadOnly Property CanSeeObject(ByVal sObKey As String) As Boolean
        {
            ...
            ElseIf Adventure.htblGroups.ContainsKey(sObKey) && Adventure.htblGroups(sObKey).GroupType = clsGroup.GroupTypeEnum.Objects Then
...
set(ByVal Integer)
    ...
    public void lStart(bool bRestart = false)
    {
        if (Status = StatusEnum.NotYetStarted || Status = StatusEnum.Finished || (Status = StatusEnum.Running && bRestart))
        {
            ...
            DoAnySteps()
            DoAnySubWalks() ' was commented out, but event has this here...
        Else
    ...
    public void lStop(bool bRunSubEvents = false, bool bReachedEnd = false)
    {
        ...
        if (Me.bLoop)
        {
            If bReachedEnd Then ' Only restart if walk comes to and end and it is set to loop - not if it is terminated by task change
    ...
    public void IncrementTimer()
    {
        ...
        If Status = StatusEnum.Running Then UserSession.DebugPrint(ItemEnum.Character, sKey, DebugDetailLevelEnum.High, "Walk " + Description + " [" + TimerFromStartOfWalk + "/" + Length + "]")

        if (TimerToEndOfWalk > 0 && TimerFromStartOfWalk > 0)
        {
            DoAnySteps()
            DoAnySubWalks()
        }
                ...
                if (iStepLength = TimerFromStartOfWalk)
                {
                    ...
                    if (sDestination = HIDDEN || Adventure.htblLocations.ContainsKey(sDestination))
                    {
                        With Adventure.htblCharacters(sKey);
                            ...
                            {
                                if (.Location.Key = Adventure.Player.Location.Key)
                                {
                                    ...
                                    if (Adventure.htblLocations(Adventure.Player.Location.Key).IsAdjacent(sDestination))
                                    {
                                        private string sTheDirection = Adventure.htblLocations(.Location.Key).DirectionTo(sDestination);
                                ...
                                ElseIf sDestination = Adventure.Player.Location.Key Then
                                    ...
                                    if (Adventure.htblLocations(Adventure.Player.Location.Key).IsAdjacent(.Location.Key))
                                    {
                                        private string sTheDirection = Adventure.htblLocations(sDestination).DirectionTo(.Location.Key);
