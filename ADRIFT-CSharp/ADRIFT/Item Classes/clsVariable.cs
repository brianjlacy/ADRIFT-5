...
#Region "Expressions"
    ...
    private void PrintTokens(TokenType CurrentToken = null)
    {
    ...
    private int PrintToken(int iToken, TokenType CurrentToken = null)
    {

        With Token(iToken);
            private bool bCurrent = .Left = CurrentToken.Left && .Right = CurrentToken.Right && .Token = CurrentToken.Token;
    ...
    private int NoRepeatRandom(int iMin, int iMax)
    {

        private string sDictKey = iMin + "-" + iMax;

        If ! Adventure.dictRandValues.ContainsKey(sDictKey) Then Adventure.dictRandValues.Add(sDictKey, New List(Of Integer))

        private List<int> lValues = Adventure.dictRandValues(sDictKey);
        ...
        private int iIndex = Random(lValues.Count - 1);
 ...
 + optoken.Token = "comma" + righttoken.Token = "expr" + rptoken.Token = "rp" Then;
                                    ...
                                    if (functoken.Token = "funct" And lptoken.Token = "lp" && lefttoken.Token = "expr" && functoken.Value = "oneof")
                                    {
                                        ' Can have any number of arguments: oneof([expr,])

                                        private int iLength = 1;
                                        private TokenType oToken = lefttoken;

                                        while (oToken.Right <> -1)
                                        {
                                            private TokenType oNext = Token(oToken.Right);
                                            ...
                                            switch (oNext.Token)
                                            {
                                                case "rp":
                                                    ...
                                                    rptoken = oNext
                                                    private int iOneOfIndex = Random(iLength - 1) + 1;
                                                    private TokenType oRandom = functoken;
                                                ...
                                                case "comma":
                                                    {
                                                    private TokenType oExpr = Token(oNext.Right);
                                                    if (oExpr.Token = "expr")
                                                    {
                                                        iLength += 1;
                        ...
                        if (lefttoken.Token = "expr" And optoken.Value = "AND" And righttoken.Token = "expr" And run = 3)
                        {
                            addtoken("expr", lefttoken.Value + righttoken.Value, lefttoken.Left, righttoken.Right);
                            If lefttoken.Left <> -1 Then Token(lefttoken.Left).Right = iToken - 1
                            If righttoken.Right <> -1 Then Token(righttoken.Right).Left = iToken - 1
                    ...
                    if (optoken.Token = "op" And righttoken.Token = "expr")
                    {
                        switch (optoken.Value)
                        {
                            ...
                            case "+" ' Not really expected:
                            case but I guess +X = X:
                                {
                                If run = 2 Then addtoken("expr", (Val(righttoken.Value)).ToString, optoken.Left, righttoken.Right)
                        }
                        if (run = 2)
