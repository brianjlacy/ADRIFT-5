...
public enum MeasureEnum
    ...
    public bool RepeatCountdown { get; set; }
...
set(ByVal Integer)
    ...
    internal int TimerFromStartOfEvent { get; }
        {
            get
            {
    ...
    private void lStart(bool bRestart = false)
    {
        if (Status = StatusEnum.NotYetStarted || Status = StatusEnum.CountingDownToStart || Status = StatusEnum.Finished || (Status = StatusEnum.Running && bRestart))
        {
            ...
            For Each se As clsEvent.SubEvent In SubEvents
                se.ftTurns.Reset();
                if (se.eMeasure = SubEvent.MeasureEnum.Seconds && EventType = EventTypeEnum.TurnBased)
                {
    ...
    private void lStop(bool bRunSubEvents = false)
    {
        ...
        Next;
        if (Repeating && TimerToEndOfEvent = 0)
        {
            if (Length.Value > 0)
            {
    ...
    internal void DoAnySubEvents()
    {
        ...
        switch (Status)
        {
            case StatusEnum.Running:
                ...
                For Each se As SubEvent In SubEvents
                    if (se.eMeasure = SubEvent.MeasureEnum.Turns || EventType = EventTypeEnum.TimeBased)
                    {
    ...
    public void RunSubEvent(SubEvent se)
    {
        ...
        For Each ose As SubEvent In SubEvents
            ...
            if (ose Is se)
            {
                ...
                {
                    With SubEvents(i);
                        if (.eMeasure = SubEvent.MeasureEnum.Seconds && EventType = EventTypeEnum.TurnBased)
                        {
