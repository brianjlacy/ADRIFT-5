...
public class clsTask
{
    ...
    internal Boolean ' Multiple Matching ContinueToExecuteLowerPriority { get; set; }
...
public enum BeforeAfterEnum
    ...
    public BeforeAfterEnum eDisplayCompletion = BeforeAfterEnum.After;
...
set(ByVal Value As String)
    ...
    public string TaskCommand(clsTask task, bool bReplaceSpecifics = true, int iMatchedTask = -1)
    {
...
set(ByVal Value As Boolean)
    ...
    public Boolean = True AggregateOutput { get; set; }
                ...
                if (sKey1 IsNot null || eType = RestrictionTypeEnum.Expression)
                {
                    switch (eType)
                    {
                        ...
                        case RestrictionTypeEnum.Character:
                            ...
                            switch (eCharacter)
                            {
                                ...
                                case CharacterEnum.BeInSameLocationAsObject:
                                    {
                                    if (sKey2.StartsWith("ReferencedObject") || Adventure.GetTypeFromKeyNice(sKey2) = "Object")
                                    {
                                        sSummary &= "be in the same location as " + Adventure.GetNameFromKey(sKey2);
                                    Else
                        ...
                        case RestrictionTypeEnum.Variable:
                            ...
                            if (sKey1.StartsWith("ReferencedNumber"))
                            {
                        ...
                        case RestrictionTypeEnum.Property:
                            ...
                            if (Adventure.htblAllProperties.TryGetValue(sKey1, p))
                            {
                                ...
                                switch (p.Type)
                                {
                                    ...
                                    case clsProperty.PropertyTypeEnum.ValueList:
                                        ...
                                        if (p.Type = clsProperty.PropertyTypeEnum.ValueList && IsNumeric(StringValue))
                                        {
                                            For Each sValue As String In p.ValueList.Keys
                                                if (p.ValueList(sValue) = SafeInt(StringValue))
                                                {
                                                    sSummary &= "'" + sValue + "'";
                                                    bSetVar = True
                                                    Exit For;
                                        ...
                                        If ! bSetVar Then sSummary &= "'" + StringValue + "'"
            ...
            try
            {
                switch (eItem)
                {
                    ...
                    case ItemEnum.SetProperties:
                        {
                        private clsProperty p = null;
                        if (Adventure.htblAllProperties.TryGetValue(sKey2, p))
                        {
                            ...
                            if (Not bSetVar && p.Type = clsProperty.PropertyTypeEnum.ValueList && IsNumeric(sPropertyValue))
                            {
                                For Each sValue As String In p.ValueList.Keys
                                    if (p.ValueList(sValue) = SafeInt(sPropertyValue))
                                    {
                                        sSummary &= "'" + sValue + "'";
                                        bSetVar = True
                                        Exit For;
                            ...
                            if (Not bSetVar && sPropertyValue.StartsWith("Referenced"))
                            {
                                sSummary &= Adventure.GetNameFromKey(sPropertyValue);
                                bSetVar = True
                            }
