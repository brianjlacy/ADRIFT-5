    ...
    public string ViewLocation { get; }
        {
            ...
            if (htblObjects.Count > 0)
            {
                if (sView <> "" || Adventure.dVersion < 5)
                {
                    ...
                    sView &= "Also here is " + htblObjects.List(, , false) + ".";
                Else
                    sView &= "There is " + htblObjects.List(, , false) + " here.";
                }
            }
...
#if Runner
            private string sLoc = Adventure.Player.Location.LocationKey;
            For Each ch As clsCharacter In Adventure.htblCharacters.Values
                If ch != Adventure.Player && ch.IsVisibleAtLocation(sLoc) Then htblCharactersInLocation.Add(ch, ch.Key)
            Next;
#endif
